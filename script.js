...
function addUsbFile(url) {
  ...
  if (url.match(/\.(jpg|jpeg|png|webp|gif)$/i)) {
    ...
    preview = `<img src="icons/file.png">`;
  }
...
document.querySelectorAll(".xp-folder").forEach(folder => {
...
function addUsbFile(url) {
  ...
  if (url.match(/\.(jpg|jpeg|png|webp|gif)$/i)) {
    usbFiles.photos.push({ url, name, date });
  } else if (url.match(/\.(mp4|webm|mov)$/i)) {
    usbFiles.videos.push({ url, name, date });
  }
...
function renderUsbFiles() {
  ...
  usbFiles[currentFolder].forEach(file => {
    const div = document.createElement("div");
    div.className = "xp-file";

    const preview = file.url.match(/\.(mp4|webm|mov)$/i)
...
document.getElementById("usbIcon").addEventListener("click", () => {
  document.getElementById("usbModal").style.display = "block";
});
